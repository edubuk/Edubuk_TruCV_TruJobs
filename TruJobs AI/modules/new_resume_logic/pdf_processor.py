...
import time
import uuid
from config import BUCKET_NAME, RESUME_PREFIX, RESUME_TEXT_PREFIX
...
#8. S3 Text Upload (for JSON resume uploads)
'''
Purpose: Saves plain text (flattened from JSON resume) to the configured S3 bucket
under a separate prefix for audit and traceability.
'''
def save_text_to_s3(text_content, text_filename):
    """Save text content to S3 bucket as UTF-8 .txt file"""
    try:
        if not isinstance(text_content, (str, bytes)):
            text_content = str(text_content)
        body = text_content.encode('utf-8') if isinstance(text_content, str) else text_content
        s3_key = f"{RESUME_TEXT_PREFIX}{text_filename}"
        s3.put_object(
            Bucket=BUCKET_NAME,
            Key=s3_key,
            Body=body,
        ...
        logger.error(f"S3 text upload error: {str(e)}")
        raise