#!/usr/bin/env python3
"""
Test the fixed JSON resume flattening with Ajeet's exact resume structure
"""
...
# Ajeet's exact resume JSON structure
AJEET_RESUME_JSON = {
    "name": "Ajeet Verma",
    "contact": {
...
def main():
    ...
    print('Testing fixed JSON resume flattening with Ajeet\'s resume structure...')
    ...
    report: Dict[str, Any] = {
        'test_description': 'Testing fixed JSON flattening for complex nested resume structure',
        'http_status': resp.status_code,
        'api_time_sec': dt,
        'response': None,
        's3_verification': None,
        'opensearch_verification': None,
        'original_payload_size': len(json.dumps(payload))
    }
    ...
    if resp.status_code == 200:
        ...
        if s3_key:
            ...
            if s3_info.get('exists'):
                ...
                preview = s3_info.get('content_preview', '')[:500]
                print(f'   "{preview}..."')
            else:
                print(f'   ❌ S3 object not found: {s3_info.get("error")}')
        
        if resume_id:
            print('🔍 Verifying OpenSearch indexing...')
    ...
    out_path = out_dir / f'ajeet_resume_fix_test_{int(time.time())}.json'
