...
from pdf_processor import extract_text_from_pdf, save_pdf_to_s3, save_text_to_s3
...
def enhance_metadata_with_achievements(metadata, text):
    """Enhance metadata to include courses and awards as certifications"""
    try:
        # Make a copy to avoid modifying the original
        enhanced = metadata.copy()
        
        # Get existing certifications
        existing_certs = enhanced.get('certifications', [])
        if not isinstance(existing_certs, list):
            existing_certs = []
        
        additional_certs = []
        
        # Extract courses and awards from the flattened text
        # Look for course patterns
        course_patterns = [
            r'Course:\s*Course:\s*([^;]+)',  # "Course: Course: Full stack dev"
            r'Course:\s*([^;]+?)(?:;|Organization:|Duration:|Description:|$)',
            r'course_name["\']?:\s*["\']?([^"\',;]+)["\']?',  # JSON field
        ]
        ...
        for pattern in course_patterns:
            ...
            for match in matches:
                ...
                if course_name and len(course_name) > 2 and course_name not in additional_certs:
                    additional_certs.append(f"Course: {course_name}")
        
        # Look for award patterns
        award_patterns = [
            r'Achievement:\s*Award:\s*([^;]+)',  # "Achievement: Award: IEEE"
            r'Award:\s*([^;]+?)(?:;|Organization:|Date:|Description:|$)',
            r'award_name["\']?:\s*["\']?([^"\',;]+)["\']?',  # JSON field
        ]
        ...
        for pattern in award_patterns:
            ...
            for match in matches:
                ...
                if award_name and len(award_name) > 2 and award_name not in additional_certs:
                    additional_certs.append(f"Award: {award_name}")
        
        # Look for certification patterns in the text
        cert_patterns = [
            r'Achievement:\s*([^;]+?)(?:;|Organization:|Date:|$)',
            r'Course:\s*([^;]+?)(?:;|Organization:|Duration:|$)',
            r'Award:\s*([^;]+?)(?:;|Organization:|Date:|$)',
        ]
        ...
        for pattern in cert_patterns:
            ...
            for match in matches:
                ...
                if (cert_text and len(cert_text) > 2 and 
                    not cert_text.startswith('Course:') and 
                    not cert_text.startswith('Award:') and
                    cert_text not in additional_certs):
                    additional_certs.append(cert_text)
        
        # Combine existing and additional certifications
        all_certs = existing_certs + additional_certs
        ...
        for cert in all_certs:
            ...
            if cert_lower not in seen and cert.strip():
                unique_certs.append(cert.strip())
                seen.add(cert_lower)
        
        enhanced['certifications'] = unique_certs
        
        if additional_certs:
            logger.info(f"✅ Enhanced certifications with {len(additional_certs)} courses/awards: {additional_certs}")
        
        return enhanced
        
    except Exception as e:
        logger.warning(f"⚠️ Error enhancing metadata with achievements: {e}")
        return metadata  # Return original if enhancement fails

def extract_nano_id_from_text(text):
    """Extract nano_Id from the flattened resume text"""
    try:
        # Look for nano_Id patterns in the text
        nano_id_patterns = [
            r'Nano ID:\s*([^\n\r]+)',  # "Nano ID: Ajeet-Verma-8LgGByRevQmeWdwp"
            r'nano_Id["\']?:\s*["\']?([^"\',\n\r]+)["\']?',  # JSON field
            r'nanoId["\']?:\s*["\']?([^"\',\n\r]+)["\']?',   # Alternative JSON field
        ]
        
        for pattern in nano_id_patterns:
            matches = re.findall(pattern, text, re.IGNORECASE)
            for match in matches:
                nano_id = match.strip()
                if nano_id and len(nano_id) > 3:  # Basic validation
                    logger.info(f"✅ Extracted nano_Id: {nano_id}")
                    return nano_id
        
        logger.info("ℹ️ No nano_Id found in resume text")
        return None
        
    except Exception as e:
        logger.warning(f"⚠️ Error extracting nano_Id: {e}")
        return None
...
#9. Handling Different Input Types
        ...
        if input_type == 'json':
            ...
            # Persist JSON resume text to S3 for audit/traceability
            try:
                text_filename = f"{resume_id}.txt"
                s3_key = save_text_to_s3(text, text_filename)
                logger.info(f"✅ Saved JSON resume text to S3: {s3_key}")
            except Exception as e:
                logger.warning(f"⚠️ Failed to save JSON resume text to S3: {e}")
...
#10. Save PDF to S3
        ...
        # s3_key may be set earlier for JSON uploads; initialize if absent
        s3_key = locals().get('s3_key', None)
        if pdf_content:
...
#11. AI Metadata Extraction
        ...
        # Enhance metadata to include courses and awards as certifications
        enhanced_metadata = enhance_metadata_with_achievements(raw_metadata, text)
        
        # Extract nano_Id from the text if present
        nano_id = extract_nano_id_from_text(text)
        
        bedrock_time = time.time() - bedrock_start
        candidate_name = enhanced_metadata.get('full_name', 'Unknown')
        logger.info(f"Extracted and enhanced metadata for candidate: {candidate_name} in {bedrock_time:.2f}s")
        logger.info(f"📜 Total certifications (including courses/awards): {len(enhanced_metadata.get('certifications', []))}")

        # Normalize metadata for OpenSearch compatibility
        normalized_metadata = normalize_metadata_for_opensearch(enhanced_metadata, text) # opensearch_client module

#12. Create Section-Specific Embeddings
        # Create section-specific embeddings
        embedding_start = time.time()
        embeddings = create_section_embeddings(enhanced_metadata) # ai_services module
...
#14. Index document in OpenSearch
        ...
        response = index_resume_document(
            ...
            candidate_name, s3_key, normalized_metadata, embeddings, nano_id
        )
...
# 15. Final timeout check
    ...
    #15. Exception Handling ,errors handling
